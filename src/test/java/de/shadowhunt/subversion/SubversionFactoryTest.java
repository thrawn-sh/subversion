...
public class SubversionFactoryTest {
	...
	public void assertSupportedSchemeFileTest() {
		...
		RepositoryFactory.assertSupportedScheme(uri);
	...
	public void getInstanceTest() {
		final URI uri = URI.create("http://subversion.example.net/svn/test-repo");
		final Repository repository = RepositoryFactory.getInstance(uri, true, Version.HTTPv1);
	...
	public void getInstanceUnsupportedServerVersionTest() {
		final URI uri = URI.create("http://subversion.example.net/svn/test-repo");
		RepositoryFactory.getInstance(uri, true, null);
		Assert.fail("Version must not be null");
	}

	@Test
	public void removeEndingSlash() {
		final URI withoutSlash = URI.create("https://subversion.example.net/svn/test-repo");
		Assert.assertEquals("uri without ending slash are not modified", withoutSlash, RepositoryFactory.removeEndingSlash(withoutSlash));

		final URI withSlash = URI.create("https://subversion.example.net/svn/test-repo/");
		Assert.assertEquals("uri without ending slash are not modified", withoutSlash, RepositoryFactory.removeEndingSlash(withSlash));
	}
}
